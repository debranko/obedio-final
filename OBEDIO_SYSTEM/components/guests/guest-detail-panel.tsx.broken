...
export function GuestDetailPanel({
  ...
  const handleImageUpdate = async (imageDataUrl: string | null) => {
    ...
    try {
      ...
      // Automatski sačuvaj promenu ako trenutno nismo u modu za uređivanje
      if (!isEditing) {
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-3">
              <div className="relative group">
                ...
                <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 rounded-full flex items-center justify-center transition-all duration-200 opacity-0 group-hover:opacity-100" onClick={(e) => {
                  e.stopPropagation();
                  ...
                  <div>
                    <div className="flex space-x-2">
                      <div className="flex-1">
                        <Label className="text-xs text-muted-foreground">Guest Tag</Label>
                        <div className="font-medium">
                          {guestTag || <span className="text-muted-foreground italic">No tag</span>}
                        </div>
                      </div>
                      <div className="flex-1">
                        <Label className="text-xs text-muted-foreground">Party Size</Label>
                        <div className="font-medium">{partySize || 1}</div>
                      </div>
                    </div>
                  </div>
                      ...
                      {serviceHistory.map((service) => (
                        <div key={service.id} className="border rounded-md p-3">
                          ...
                          <div className="text-sm text-muted-foreground">
                            {format(new Date(service.timestamp), "MMM dd, yyyy HH:mm")}
                          </div>
                          {service.description && (
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                      ...
                      <div className="bg-muted/30 p-3 rounded-md">
                        ...
                        {isEditing && (
                          <div className="text-xs text-muted-foreground mt-1 mb-1">
                            Enter food preferences separated by commas
                            <span className="ml-1 text-gray-400 cursor-help" title="E.g. Fish, Vegetarian, Meat">ℹ️</span>
                          </div>
                        )}
                        {!isEditing ? (
                          <div>
                            {editedPreferences.food && editedPreferences.food.length > 0 ? (
                              ...
                              <div className="text-sm text-muted-foreground italic">No food preferences defined</div>
                            )}
                          </div>
                        ) : (
                          <div>
                            <Input
                              ...
                              onChange={(e) =>
                                setEditedPreferences({
                                  ...editedPreferences,
                                  food: e.target.value ? e.target.value.split(",").map((item) => item.trim()) : [],
                                })
                              }
                              placeholder="E.g. Fish, Meat, Vegetarian"
                            />
                            {editedPreferences.food.length > 0 && (
                              <div className="mt-2 flex flex-wrap gap-1">
                                {editedPreferences.food.map((food, index) => (
                                  <Badge key={index} variant="outline" className="bg-green-50 text-green-700">
                                    {getFoodEmoji(food)} {food}
                      ...
                      <div className="bg-muted/30 p-3 rounded-md">
                        ...
                        {isEditing && (
                          <div className="text-xs text-muted-foreground mt-1 mb-1">
                            Enter drink preferences separated by commas
                            <span className="ml-1 text-gray-400 cursor-help" title="E.g. Coffee, Tea, Wine">ℹ️</span>
                          </div>
                        )}
                        {!isEditing ? (
                          <div>
                            {editedPreferences.drinks && editedPreferences.drinks.length > 0 ? (
                              ...
                              <div className="text-sm text-muted-foreground italic">No drink preferences defined</div>
                            )}
                          </div>
                        ) : (
                          <div>
                            <Input
                              ...
                              onChange={(e) =>
                                setEditedPreferences({
                                  ...editedPreferences,
                                  drinks: e.target.value ? e.target.value.split(",").map((item) => item.trim()) : [],
                                })
                              }
                              placeholder="E.g. Coffee, Tea, Wine"
                            />
                            {editedPreferences.drinks.length > 0 && (
                              <div className="mt-2 flex flex-wrap gap-1">
                                {editedPreferences.drinks.map((drink, index) => (
                                  <Badge key={index} variant="outline" className="bg-blue-50 text-blue-700">
                                    {getDrinkEmoji(drink)} {drink}
                      ...
                      <div className="bg-muted/30 p-3 rounded-md">
                        <div className="flex justify-between items-start">
                          <Label className="text-xs text-muted-foreground">Room Temperature</Label>
                          {isEditing ? (
                            <div className="text-sm font-medium">{editedPreferences.roomTemperature}°C</div>
                          ) : (
                            <div className="text-sm font-medium">{editedPreferences.roomTemperature}°C</div>
                          )}
                        </div>
                          ...
                          <div className="text-sm mt-1">
                            <Badge
                              ...
                              className={`flex items-center ${
                                editedPreferences.roomTemperature < 21
                                  ? "bg-blue-50 text-blue-700"
                                  : editedPreferences.roomTemperature > 25
                      ...
                      {/* Cleaning time */}
                      <div className="bg-muted/30 p-3 rounded-md">
                        <Label className="text-xs text-muted-foreground">Preferred Cleaning Time</Label>
                        {isEditing ? (
                          <Select
